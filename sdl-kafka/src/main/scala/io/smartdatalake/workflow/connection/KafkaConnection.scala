...
case class KafkaConnection(
    ...
    authMode match {
      ...
      case Some(m: SASLSCRAMAuthMode) =>
        ...
        props.setProperty("sasl.jaas.config",
          s"""$jaasLoginModule required username="${m.username.resolve()}" password="${m.passwordSecret.resolve()}";""")
        ...
        m.truststorePath.foreach { truststorePath =>
          ...
          props.setProperty(
            ...
            m.truststorePassSecret.map(_.resolve()).getOrElse(
              throw ConfigurationException("truststorePassSecret must be set if truststorePath is set.")
            )
          )
          props.setProperty(SslConfigs.SSL_TRUSTSTORE_TYPE_CONFIG, m.truststoreType)
        }
  ...
  private[workflow] val sparkOptions = (authOptions ++ options).concat(
    Map(KafkaConfigOptionPrefix + ConsumerConfig.BOOTSTRAP_SERVERS_CONFIG -> brokers)
  )
