---
title: Integrating SDLB with Databricks
...
The [Databricks](https://databricks.com) platform provides an easy accessible and configurable way to implement a modern analytics platform. 
...
1. **Databricks Setup** \
[**Databricks**](https://databricks.com) accounts can be created in various ways. This guide has been tested using a [Free Trial](https://databricks.com/try-databricks) with the AWS backend. For this, we created an account using an email-address with Root User sign in.
...
2. **Cluster Setup** \
In the Databricks UI, create a compute cluster. Pay attention to the following settings:
       ...
       Note that only the major and minor versions must match, patch versions can differ. For example, SDLB Version `2.7.1` uses Spark `3.5.2` with Scala `2.12.15`. Alternatively, there are other SDLB builds for different Spark versions, see also [Architecture](../../docs/architecture) for supported versions.  
...
3. **Catalog Setup** \
Setup a Catalog to store our data.
      - Go to the `Catalog` menu and create a new catalog called `my_catalog`.
      ...
      - Why do we need a volume? Because we need a place to store the Jar-File (Java Archive) of SDLB, as well as files storing the state of SDLB runs. 
      This place needs to be accessible from the Spark Cluster Nodes (Driver and Executors), as well as from any Databricks Notebook. 
...
4. **Get the code and run Part 1 of the Notebook**         
      ...
      - `Run` the first cell to have the buttons appear. Then, fill the 3 parameters REPODIR, TMPDIR, VOLDIR with your values 
...
Let's get some data. In the `Workflows` tab, click on `Create Job` and under `Tasks` click on `+ Add task`.
