...
private[smartdatalake] trait SchemaValidation { this: DataObject =>
  ...
  def schemaMin: Option[SnowparkStructType]

  def validateSchemaMin(df: SnowparkDataFrame, role: String): Unit = {
  }

  def validateSchema(df: SnowparkDataFrame, schemaExpected: SnowparkStructType, role: String): Unit = {
  }
}
