...
/**
 * Tests HistorizeAction with SparkConnectTableDataObjects, using the shared HistorizeActionBehaviour.
 ...
 */
class SparkConnectHistorizeActionTest extends AnyFunSuite with Matchers
  ...
  private def createSrcDataObject(id: String, registry: InstanceRegistry) =
    SparkConnectTableDataObject(id, Table(Some("default"), s"sdlb_hist_$id"),
  ...
  private def createTgtDataObject(id: String, primaryKey: Option[Seq[String]], registry: InstanceRegistry) =
    SparkConnectTableDataObject(id, Table(Some("default"), s"sdlb_hist_$id", primaryKey = primaryKey),
      format = Some("delta"), allowSchemaEvolution = true, connectionId = defaultEngineConnection.id)(registry)

  testsFor(historizeWithMergeMode(createSrcDataObject, createTgtDataObject))

  testsFor(activateMergeMode(createSrcDataObject, createTgtDataObject))
}
