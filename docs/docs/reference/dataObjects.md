...
* database based: JDBC, DeltaLake, Iceberg, Hive, Spark Connect catalog tables, ...
* connectors: Airbyte, sFTP, Webservice, ...
* machine learning: MLflow experiments, see [MLflow Actions](actions/mlflow)

If necessary, additional formats can be implemented by providing an own DataObject implementation (Java/Scala), see [Extending SDLB](extending), or implementing an Airbyte connector (Python).
