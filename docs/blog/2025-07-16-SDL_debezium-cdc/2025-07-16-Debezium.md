...
- starts the validity of a new version at `_commit_timestamp`, e.g. the history follows the time axis of the source database rather than the time SDLB happened to run. Set `mergeModeCDCUseSourceTimestamp = false` to use the runs reference timestamp instead,
