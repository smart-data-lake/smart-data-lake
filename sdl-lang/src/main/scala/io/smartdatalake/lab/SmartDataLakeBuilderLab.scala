...
import io.smartdatalake.workflow.action.spark.customlogic.{CustomTransformMethodDef, CustomTransformMethodWrapper, NotFoundError}
 ...
 * @param dataObjectCatalogFactory A method to create a action catalog instance to be used by this SmartDataLakeBuilderLab.
 *                                 Note that this is normally an instance from a class generated by LabCatalogGenerator in a second compile phase.
...
case class SmartDataLakeBuilderLab[D,A](
  ...
  case class GetDataFrameBuilder private (
                                           ...
                                           transformers: Seq[CustomTransformMethodDef] = Seq(),
     ...
     */
    def withTransformer(transformer: CustomTransformMethodDef): GetDataFrameBuilder = copy(transformers = transformers :+ transformer)
  ...
  private def sparkTransform(transformer: CustomTransformMethodDef, partitionValues: Seq[PartitionValues], filters: Map[String,Column], options: Map[String,String] = Map(), previousDfs: Map[String,DataFrame] = Map()): Map[String,DataFrame] = {
    require(transformer.customTransformMethod.isDefined, s"${transformer.getClass.getSimpleName} transform method is not overridden")
    val methodWrapper = new CustomTransformMethodWrapper(transformer.customTransformMethod.get)
    require(methodWrapper.returnsSingleDataset || methodWrapper.returnsMultipleDatasets, s"The return type of the transform method is ${transformer.customTransformMethod.get.returnType} but should be one of DataFrame, Dataset[_], Map[String,DataFrame] or Map[String,Dataset[_]]")

    // get DataFrames
    val previousDfsTolerantKey = previousDfs.map{case (id, df) => (prepareTolerantKey(id), (id, df))}
    var dfs = previousDfs ++ methodWrapper.getInputDataObjectNames.map{
      case (name, param) =>
        previousDfsTolerantKey.getOrElse(name, {
          val dataObject = dataObjectsTolerantKey.getOrElse(name, throw NotFoundError(s"No DataObject found with name $name for parameter ${param.name}"))
    ...
    if (methodWrapper.returnsSingleDataset) optionsPrep += (OPTION_OUTPUT_DATAOBJECT_ID -> DEFAULT_DATAOBJECT_ID)
    methodWrapper.call(transformer, dfs, optionsPrep.toMap)(session)
  }
}
