...
case class LabSparkDfsActionWrapper[A <: CustomDataFrameAction](action: A, context: ActionPipelineContext)
    extends LabSparkActionWrapper[A, GenericDfsTransformerDef, CustomDfsTransformer](action, context) {
  override private[smartdatalake] def transform(
      inputSubFeeds: Seq[DataFrameSubFeed],
      outputSubFeeds: Seq[DataFrameSubFeed],
      selectedTransformerIndexes: Option[Seq[Int]],
      additionalTransformers: Seq[GenericDfsTransformerDef],
      replacedTransformers: Map[Int, GenericDfsTransformerDef],
      additionalTransformerOptions: Map[Int, Map[String, String]]
  ): Map[String, GenericDataFrame] = {
    val mainPartitionValues = action.getMainPartitionValues(inputSubFeeds)
    val transformers = action.transformers
    val selectedTransformerIndexesPrep =
      selectedTransformerIndexes.map(_.filter(_ < transformers.size)).getOrElse(transformers.indices).toSet
    val transformersToApply = transformers.zipWithIndex
      .filter { case (_, idx) => selectedTransformerIndexesPrep.contains(idx) }
      .map { case (t, idx) =>
        replacedTransformers.getOrElse(idx, t) match {
          ...
          case t if additionalTransformerOptions.isDefinedAt(idx) =>
            throw new IllegalStateException(
              s"additionalTransformerOptions defined for idx $idx but transformer of type ${t.getClass.getSimpleName} can not handle options"
            )
          case t => t
        }
...
case class LabSparkDfActionWrapper[A <: DataFrameOneToOneActionImpl](action: A, context: ActionPipelineContext)
    extends LabSparkActionWrapper[A, GenericDfTransformerDef, CustomDfTransformer](action, context) {
  override private[smartdatalake] def transform(
      inputSubFeeds: Seq[DataFrameSubFeed],
      outputSubFeeds: Seq[DataFrameSubFeed],
      selectedTransformerIndexes: Option[Seq[Int]],
      additionalTransformers: Seq[GenericDfTransformerDef],
      replacedTransformers: Map[Int, GenericDfTransformerDef],
      additionalTransformerOptions: Map[Int, Map[String, String]]
  ): Map[String, GenericDataFrame] = {
 ...
 *   Action type
 * @tparam T:
 *   Transformer type used by this Action type
 * @tparam S:
 *   Scala Customer Transformer type used for this Action type.
 */
abstract class LabSparkActionWrapper[A <: DataFrameActionImpl, T <: Transformer, S](action: A, context: ActionPipelineContext) {

  /**
   * Get DataFrames used in Action. Parameters can be used to control if input or transformed
   * DataFrames are returned.
   *
   * @param partitionValues
   *   optional partition values used to filter input DataFrames.
   * @param filters
   *   optional yMap of column name and filter expressions, used to filter DataFrames created that
   *   contain the column name in their schema.
   * @param selectedTransformerIndexes
   *   index of transformers configured for this Action that should be applied. None -> all
   *   transformers are applied (Default).
   * @param postProcessOutputSubFeeds
   *   if post-processing of SubFeeds should be applied. Default = false. PostProcessing will filter
   *   DataFrames created by Transformations to configured outputs, and let outputs post-process
   *   SubFeeds.
   * @param additionalTransformers
   *   transformers to apply after existing transformers of the Action. Note that these transformers
   *   are appended after existing transformers are selected by using selectedTransformerIndexes.
   * @param replacedTransformers
   *   transformers to apply at the index of another configured transformers of the Action. The
   *   index of the transformer is starting from 0.
   * @param additionalTransformerOptions
   *   additional transformer options to add to transformer at given index. The index of the
   *   transformer is starting from 0.
   * @return
   *   a Map of ids and DataFrames.
   */
  def getDataFrames(
      partitionValues: Seq[PartitionValues] = Seq(),
      filters: Map[String, Column] = Map(),
    ...
    getGenericDataFrames(
      partitionValues,
      filters.view.mapValues(SparkColumn).toMap,
      selectedTransformerIndexes,
      postProcessOutputSubFeeds,
      additionalTransformers,
      replacedTransformers,
      additionalTransformerOptions
    )
  ...
  def simulateExecutionMode(partitionValues: Seq[PartitionValues] = Seq()): Option[ExecutionModeResult] =
    action.executionMode match {
      case Some(executionMode) =>
        ...
        val mainInput = action.getMainInput
        ...
        val inputSubFeeds = emptyInputSubFeeds.map { subFeed =>
          ...
          action.updateInputPartitionValues(action.inputMap(subFeed.dataObjectId), action.subFeedConverter.fromSubFeed(subFeed),
            partitionValues)
        }
  ...
  case class GetDataFrameBuilder private (
     ...
     * Note that the order of the transformers can not be changed. Default is to apply all
     * transformers.
     */
    def withLimitedTransformerNb(limit: Int): GetDataFrameBuilder = copy(selectedTransformerIndexes = Some(0 until limit))

    /**
     * Enable SubFeed postprocessing. Default is to not apply SubFeed postprocessing. PostProcessing
     * will filter DataFrames created by Transformations to configured outputs, and let outputs
     * post-process SubFeeds.
     */
     ...
     *
     * @param idx
     *   : index of the transformer, starting from 0
     */
    def withReplacedTransformer(idx: Int, transformer: T): GetDataFrameBuilder =
      copy(replacedTransformers = replacedTransformers + (idx -> transformer))
     ...
     *   : index of the transformer, starting from 0
     */
    def withReplacedTransformer(idx: Int, transformer: S): GetDataFrameBuilder =
      copy(replacedTransformers = replacedTransformers + (idx -> createLabTransformer(transformer)))

    /**
     * Add additional transformer to be applied after transformers of this action.
     */
    def withAdditionalTransformer(transformer: T): GetDataFrameBuilder =
      copy(additionalTransformers = additionalTransformers :+ transformer)

    /**
     * Add additional custom transformer to be applied after transformers of this action.
     */
    def withAdditionalTransformer(transformer: S): GetDataFrameBuilder =
      copy(additionalTransformers = additionalTransformers :+ createLabTransformer(transformer))
     ...
     *   : index of the transformer, starting from 0
     */
    def withAdditionalTransformerOptions(idx: Int, options: Map[String, String]): GetDataFrameBuilder =
      copy(additionalTransformerOptions = additionalTransformerOptions + (idx -> options))

    /**
     * Get DataFrames using selected options.
     */
    def get: Map[String, DataFrame] = {
      val dfs = getDataFrames(
        partitionValues,
        filters,
        selectedTransformerIndexes,
        postProcessOutputSubFeeds,
    ...
    override protected def setPartitionValues(partitionValues: Seq[PartitionValues]): GetDataFrameBuilder =
      copy(partitionValues = partitionValues)

    override protected def setFilters(filters: Map[String, Column]): GetDataFrameBuilder = copy(filters = filters)
  }

  private[smartdatalake] def getGenericDataFrames(
      partitionValues: Seq[PartitionValues] = Seq(),
      filters: Map[String, GenericColumn] = Map(),
      selectedTransformerIndexes: Option[Seq[Int]] = None,
      postProcessOutputSubFeeds: Boolean = false,
      additionalTransformers: Seq[T] = Seq(),
      replacedTransformers: Map[Int, T] = Map(),
    ...
    assert(
      !selectedTransformerIndexes.exists(_.isEmpty) || !postProcessOutputSubFeeds,
      "selectedTransformerIndexes=empty and postProcessOutputSubFeeds=true can not be set together"
    )
    ...
    // transform
    val outputDataFrames = transform(inputSubFeeds, outputSubFeeds, selectedTransformerIndexes, additionalTransformers,
      replacedTransformers, additionalTransformerOptions)

    // check and adapt output SubFeeds
    if (postProcessOutputSubFeeds) {
      outputSubFeeds = outputSubFeeds.map { subFeed =>
        val df = outputDataFrames.getOrElse(
          subFeed.dataObjectId.id,
          throw ConfigurationException(
            s"(${action.id}) No result found for output ${subFeed.dataObjectId}. Available results are ${outputDataFrames.keys.mkString(", ")}."
          )
        )
   ...
   */
  private[smartdatalake] def transform(
      inputSubFeeds: Seq[DataFrameSubFeed],
      outputSubFeeds: Seq[DataFrameSubFeed],
      selectedTransformerIndexes: Option[Seq[Int]],
      additionalTransformers: Seq[T],
      replacedTransformers: Map[Int, T],
      additionalTransformerOptions: Map[Int, Map[String, String]]
  ): Map[String, GenericDataFrame]
...
private case class LabSparkDfsTransformer(
    override val name: String = "labScalaSparkTransform",
    customTransformer: CustomDfsTransformer,
    options: Map[String, String] = Map(),
    runtimeOptions: Map[String, String] = Map()
) extends OptionsSparkDfsTransformer with ExcludeFromSchemaExport {
  ...
  override def transformPartitionValuesWithOptions(
      actionId: ActionId,
      partitionValues: Seq[PartitionValues],
      options: Map[String, String]
...
private case class LabSparkDfTransformer(
    override val name: String = "labScalaSparkTransform",
    customTransformer: CustomDfTransformer,
    options: Map[String, String] = Map(),
    runtimeOptions: Map[String, String] = Map()
) extends OptionsSparkDfTransformer with ExcludeFromSchemaExport {
  override val description: Option[String] = None
