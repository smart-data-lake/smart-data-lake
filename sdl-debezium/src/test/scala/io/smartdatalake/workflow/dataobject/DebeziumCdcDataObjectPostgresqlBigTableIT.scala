...
object DebeziumCdcDataObjectPostgresqlBigTableIT extends App with SmartDataLakeLogger {
  ...
  val appName = "sdlb-debezium-big-table-test"
  val feedName = "debezium-test-big"
  val tempDir = Files.createTempDirectory(feedName)
  val statePath = "target/stateTestBig/"
  ...
  jdbcConnection.execJdbcStatement("TRUNCATE demo.big_table")
  jdbcConnection.execJdbcStatement(
    """INSERT INTO demo.big_table (value)
      |SELECT 'test_' || s.id::text
