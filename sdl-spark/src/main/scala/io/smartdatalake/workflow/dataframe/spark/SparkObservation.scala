...
import io.smartdatalake.workflow.dataframe.{DataFrameObservation, NoMetricsReceivedException}
...
private[smartdatalake] class SparkObservation(name: String = UUID.randomUUID().toString) extends DataFrameObservation with SmartDataLakeLogger {
  ...
  override def linkWithInputObservations(inputObservations: Seq[io.smartdatalake.workflow.dataframe.DataFrameObservation], prefix: String): Unit = {
    val inputSparkObservationNames = inputObservations.collect { case x: SparkObservation => x }.map(_.getName)
    setOtherObservationNames(inputSparkObservationNames)
    setOtherObservationsPrefix(prefix)
  }
