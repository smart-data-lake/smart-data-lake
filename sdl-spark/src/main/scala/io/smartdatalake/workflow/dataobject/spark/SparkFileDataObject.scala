...
import io.smartdatalake.workflow.dataobject.generic.{CanCreateIncrementalOutput, ExpectationValidation, SchemaValidation, UserDefinedSchema}
...
trait SparkFileDataObject extends HadoopFileDataObject
  ...
  def afterRead(df: DataFrame)(implicit context: ActionPipelineContext): DataFrame = {
    ...
    validateSchemaHasPartitionCols(df.columns.toSeq, "read")
  ...
  def isV2ReadDataSource(implicit context: ActionPipelineContext): Boolean = {
    DataSource.lookupDataSourceV2(readFormat, getSparkSession.sessionState.conf).isDefined
  }
