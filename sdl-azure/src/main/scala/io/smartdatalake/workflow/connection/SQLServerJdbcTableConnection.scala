 ...
 * @param metadata
 */
case class SQLServerJdbcTableConnection(override val id: ConnectionId,
                                        url: String,
  ...
  override def factory: FromConfigFactory[Connection] = SQLServerJdbcTableConnection
}

object SQLServerJdbcTableConnection extends FromConfigFactory[Connection] {
  override def fromConfig(config: Config)(implicit instanceRegistry: InstanceRegistry): SQLServerJdbcTableConnection = {
    extract[SQLServerJdbcTableConnection](config)
  }
}
 ...
 */
private[smartdatalake] abstract class SQLCatalog(connection: SQLServerJdbcTableConnection) extends SmartDataLakeLogger  {
...
private[smartdatalake] object SQLCatalog {
  def fromJdbcDriver(driver: String, connection: SQLServerJdbcTableConnection): SQLCatalog = {
    driver match {
 ...
 */
private[smartdatalake] class DefaultSQLCatalog(connection: SQLServerJdbcTableConnection) extends SQLCatalog(connection) {
 ...
 */
private[smartdatalake] class OracleSQLCatalog(connection: SQLServerJdbcTableConnection) extends SQLCatalog(connection) {
  override def isDbExisting(db: String)(implicit session: SparkSession): Boolean = {
    val cntTableInCatalog = if(isQuotedIdentifier(db))  {
 ...
 */
private[smartdatalake] class SapHanaSQLCatalog(connection: SQLServerJdbcTableConnection) extends SQLCatalog(connection) {
  override def isDbExisting(db: String)(implicit session: SparkSession): Boolean = {
    val cntTableInCatalog = if(isQuotedIdentifier(db))  {
