...
/**
 * A collection of DataSets mainly used for testing
 */
object Collection extends StructTypeUtil {
  ...
  val dfIdXRows: List[(Int, Option[Double])] = List(
    (-4,  Some(NegativeInfinity)),
    (-3,  Some(PositiveInfinity)),
    (-2,  Some(NaN)),
    (-1,  None),
    (0,   Some(0.1)),
    (2,   Some(0.9999)),
  ...
  val rowsnLets: List[(String, String)] = List(
    ("1let", "Unilet"),
    ("2let", "doublet"),
    ("2let", "doublet"),
    ("3let", "triplet"),
  ...
  private val schemaIntDecimal = createStruct(Array[(String, DataType)](
      ("id",          IntegerType),
      ("deci_byte",   DecimalType(2, 0)),
      ("deci_short",  DecimalType(4, 0)),
      ("deci_int",    DecimalType(9, 0)),
      ("deci_long",   DecimalType(18, 0)),
   ...
   */
  def makeRowsWithTimeRanges[A, B](zeile: (A, B, String, String)): (A, B, Timestamp, Timestamp) =
  ...
  val dfMap: DataFrame = spark.createDataFrame(
    ArrayBuffer(Row("A", Map(0 -> 0d, 1 -> 1d))).asJava,
    schemaMap
  )
  val dfMap2: DataFrame = spark.createDataFrame(
    ArrayBuffer(Row("A", Map(0 -> Math.scalb(1d, -17), 1 -> 1d))).asJava,
    schemaMap
  )
  ...
  val rowsNonUnique: List[(String, String)] = List(
    ("1let", "unilet"),
    ("2let", "doublet"),
    ("2let", "doublet"),
    ("3let", "triplet"),
  ...
  def makeRowManyTypes(r: (Boolean, Int, Int, Int, Int, String, String, String, String, String, String, Double, Double, String, String, String)): Row =
    Row(
      ...
      r._3.shortValue(),
      r._4,
      r._5.longValue(), // BooleanType - LongType
