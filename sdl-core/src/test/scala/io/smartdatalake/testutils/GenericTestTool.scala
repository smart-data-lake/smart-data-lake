...
trait GenericTestTool extends Compare {

  def printFailedTestResultGdf(
      testName: String,
      arguments: Seq[GenericDataFrame] = Nil
  )(actual: GenericDataFrame)(expected: GenericDataFrame)(implicit logger: Logger): Unit = {
  ...
  /**
   * @param cnt
   *   number of doubles desired
   * @param minValue
   *   smallest double to return
   * @param maxValue
   *   largest double to return
   * @return
   *   a list of cnt-many equidistant doubles from minValue to maxValue
   */
  protected def getDoubleIter(cnt: Int, minValue: Double, maxValue: Double): Iterable[Double] =
    if (cnt == 0) Iterable.empty[Double]
    else {
      val ns = 1 to cnt
   ...
   */
  def testArgumentExpectedMap[K, V](experiendum: K => V, argExpMap: Map[K, V])(implicit logger: Logger): Map[K, Boolean] = {
  ...
  /**
   * testArgumentExpectedMapWithComment writes a log message decorated with provided comment in case
   * the expected value does not equal the actual
   *
   * @param experiendum
   *   map you want to test
   * @param argExpMapComm
