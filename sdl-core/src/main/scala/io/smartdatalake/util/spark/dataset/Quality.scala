...
trait Quality extends Transform {
   ...
   *   numeric column
   * @param alias
   *   suffix of resulting column names
   * @return
   *   List of aggregated columns: min, avg, max, sum
   */
  ...
  implicit class DsComment[T](ds: Dataset[T]) {
    ...
    /**
     * transformCommentCols allows to not only transform columns but also to add comments to the
     * obtained columns.
     *
     * If a DataFrame is persisted as table (e.g. hive, Databricks) the column comments are saved in
     * the corresponding MetaStore and thus visible and Databrick Catalog and DB Tools like Dbeaver.
     *
     * @param transformRenameCommentFun
     *   : function to transform, rename and comment columns
     * @param colFilter
     ...
     * @return
     *   DataFrame with transformed, renamed and commented columns
     */
    def transformCommentCols(
        transformRenameCommentFun: String => Iterable[CommentedColumn],
  ...
  implicit class DsQuality[T](ds: Dataset[T]) {
    ...
    /**
     * Converts maps to arrays and then counts distinct rows. If the dataset has a map column
     * ds.distinct() throws an exception
     *
     * @return
    ...
    def debLog(dsName: String, showRows: Boolean = false)(implicit logger: Logger): Unit = {
      ...
      if (0 < cntDistinctRows && cntRows != cntDistinctRows) {
        ...
        if (showRows) ds.getNonuniqueStats().show(8, truncate = false)
        else debugLog(s"Since showRows=$showRows duplicates are not shown.")
      }
    ...
    /**
     * @return
     *   aggregated dataFrame showing count and count distinct of each column
     */
    ...
    /**
     * Fills data gaps with either the next or the previous value. If the parameter
     * `takeNextValueFirst` is `true`, it will first look for the next value.
     *
     * WARNING: All columns in the result are nullable, even if they weren't before :(
     *
     * @param keyColNames
     *   : Columns which, together with `orderCol`, form a primary key candidate
     * @param dataColNames
     *   : Names of columns whose gaps are to be filled
     * @param orderColName
     *   : Name of the ordering column, e.g., "valid_from"
     * @param takeNextValueFirst
     *   : Indicates whether to take the next value first
     * @return
     *   A data frame without NULLs in the `dataColNames` columns
     */
    final def fillGaps(
        keyColNames: Iterable[String],
        dataColNames: Iterable[String],
        orderColName: String,
        takeNextValueFirst: Boolean = true
    ): DataFrame = {
      val fenestra_next =
        Window.partitionBy(keyColNames.head, keyColNames.tail.toSeq.toIndexedSeq: _*).orderBy(orderColName).rangeBetween(Window.currentRow,
          Window.unboundedFollowing)
      val fenestra_prev = Window.partitionBy(keyColNames.head, keyColNames.tail.toSeq.toIndexedSeq: _*).orderBy(orderColName).rangeBetween(
        Window.unboundedPreceding,
        Window.currentRow
      )

      def newColumn(colName: String): Column = if (colName.contains(colName)) {
        if (takeNextValueFirst) coalesce(first(col(colName), ignoreNulls = true).over(fenestra_next),
          last(col(colName), ignoreNulls = true).over(fenestra_prev)).as(colName)
        else {
          coalesce(last(col(colName), ignoreNulls = true).over(fenestra_prev),
            first(col(colName), ignoreNulls = true).over(fenestra_next)).as(colName)
        }
      } else {
        col(colName)
      }
     ...
     *
     * @param keyColNames
     *   : Column names of the key
     * @param fromColName
     *   : Column names of interval starts to be checked for gaps
     * @param toColName
     *   : Column names of interval ends to be checked for gaps
     * @param orderColNames
     *   : Sorting columns
     * @param gapIndicatorName
     *   : Name of the result column indicating gaps
     *
     * Notes:
     ...
     *   - `gapIndicatorName` holds a value indicating whether a gap was detected in `fromColName`
     *     or `toColName`.
     *
     * Usage:
     *   - This method fills gaps in data intervals defined by `fromColName` and `toColName`.
     *   - The `orderColNames` (e.g., `valid_from` and `valid_to`) are critical for correctly
    ...
    final def getGaps(
        ...
        fromColName: String,
        toColName: String,
        orderColNames: Iterable[String],
        gapIndicatorName: String = "_is_adjacent"
    ): DataFrame = {
  ...
  implicit class DsPk[T](ds: Dataset[T]) {
    ...
    /**
     * returns sub data frame which consists of those rows which contain at least a null in the
     * specified columns
     *
     * @param cols
     *   : names of columns which are to be considered, unspecified or empty Array mean all columns
     *   of df
     * @return
     *   sub data frame
     */
    ...
    /**
     * counts n-lets of this data frame with respect to specified columns cols. The result data
     * frame possesses the columns cols and an additional count column countColname.
     *
     * @param cols
     *   : names of columns which are to be considered, unspecified or empty Array mean all columns
     *   of df
     * @param countColname
     *   : name of count column, default name: cnt
     * @return
     *   subdataframe of n-lets
     */
    ...
    /**
     * returns nLets of this data frame with an additional count column overloaded version because
     * used often to check whether columnName forms a unique key
     *
     * @param columName
     *   : names of column which is to be considered
     * @return
     *   subdataframe of nLets with an additional count column
     */
    def getNonuniqueStats(columName: String): DataFrame = getNonuniqueStats(cols = Array(columName))

    /**
     * Returns rows of this data frame which violate uniqueness for specified columns cols. The
     * result data frame possesses an additional count column countColname.
     *
     * @param cols
     *   : names of columns which are to be considered, unspecified or empty Array mean all columns
     *   of df
     ...
     *
     * @param cols
     *   : names of columns on which the data frame is to be projected
     * @return
     *   projection of data frame df
     */
    ...
    /**
     * returns sub data frame which consists of those rows which violate PK condition for specified
     * columns
     *
     * @param cols
     *   : names of columns which are to be considered, unspecified or empty Array mean all columns
    ...
    /**
     * Checks whether the specified columns is a local minimal array of columns satisfying
     * uniqueness within the data frame
     *
     * @param cols
     *   : names of columns which are to be considered, unspecified or empty Array mean all columns
     *   of df
