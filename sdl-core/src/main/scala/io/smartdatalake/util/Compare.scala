...
trait Compare extends Serializable {
  ...
  /**
   * originMap returns the union of two sets where the elements are tagged with their origin However
   * the method here returns each element once only using the third flag LeftAndRight for elements
   * of the intersection originMap is needed to compare to maps, cf. mapAlmostSymDiff
   *
   * @param sL
   *   left set of type A
   * @param sR
   ...
   *   type of elements
   * @return
   *   map tagging the elements of sl union sR with their origin: LeftAndRight, Left or Right
   */
   ...
   */
  final def almostEqual[A](epsilon: A, x: A, y: A, relative: Boolean = false)(implicit num: Numeric[A]): Boolean = {
   ...
   *   pair of values
   * @param relative
   *   if true then comparison is done with respect to the first element
   * @param num
   *   implicit proof that type A is numeric
   * @tparam A
   *   type of elements x and y
   * @return
   *   boolean telling you whether the two elements are considered to equal
   */
  ...
  /**
   * mapAlmostSymDiff compares two maps. Two maps differ when a key belongs to one keySet only, or
   * if it belongs to both keySets but its values differ.
   *
   * @param epsilon
   *   : threshold up to two values are considered almost equal
   * @param f
   *   : left function to compare with g
   * @param g
   *   : right function to compare with f
   * @param num
   *   : proof that V is a numeric type
   * @tparam K
   *   : type of the keys
   * @tparam V
   *   : type of the values
   * @return
   *   map containing the keys and both optional values where f and g differ
   */
  final def mapAlmostSymDiff[K, V](epsilon: V)(f: Map[K, V], g: Map[K, V])(implicit
      num: Numeric[V]
  ): Map[K, (OriginTag, Option[V], Option[V])] = originMap(f.keySet, g.keySet)
  ...
  /**
   * checks whether 2 values of type A are ordered as desired note that ∀ p,q:Bool . ¬(p=¬q) = (p=q)
   * (used here in case of ¬strict)
   *
   * @param increasing
   *   whether x is supposed to be smaller (or equal) xNext
   * @param strict
   *   whether equality is good
   * @param x
   *   first value
   * @param xNext
   *   second value
   ...
   *   type of values
   * @return
   *   boolean value indicating whether they are ordered
   */
  ...
  final def isOrdered[A](increasing: Boolean, strict: Boolean, valuePair: (A, A))(implicit ord: Ordering[A]): Boolean =
    isOrdered(increasing, strict)(valuePair._1, valuePair._2)(ord)
}
