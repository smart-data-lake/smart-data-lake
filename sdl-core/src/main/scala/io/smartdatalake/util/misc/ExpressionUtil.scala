 ...
 * Spark expressions.
 */
private[smartdatalake] object ExpressionUtil {

  private val tokenStartChar = "%"
  private val tokenExpressionRegex = (tokenStartChar + """\{(.*?)\}""").r

  /**
   * Substitutes all tokens in a string by the expression defined by the token evaluated against the
   * given case class instance. Token syntax: "%{<spark sql expression>}"
   *
   * @param id
   ...
   * @param str
   *   String with tokens to replace
   * @param data
   *   Case class instance with data to be used as replacement
   */
   ...
   *
   * @param id
   *   id of the config object for meaningful exception text
   * @param configName
   *   optional configuration name for meaningful exception text
   * @param expression
   ...
   */
  def evaluateBoolean[T <: Product: TypeTag](
      id: ConfigObjectId,
   ...
   *   optional configuration name for meaningful exception text
   * @param expression
   *   expression to be evaluated as Spark column
   * @param data
   *   case class instance
   * @tparam T
   *   class of object the expression should be evaluated on
  ...
  def evaluate[T <: Product: TypeTag, R: TypeTag: ClassTag](id: ConfigObjectId, configName: Option[String], expression: String, data: T): Option[R] =
    try {
      ...
      case e: Exception =>
        throw ConfigurationException(s"($id) spark expression evaluation for '$expression'${getConfigNameMsg(configName)} failed: ${e.getMessage}", configName,
          e)
    }
   ...
   *   expression to be evaluated
   * @param data
   *   a list of case class instances
   * @tparam T
   *   class of object the expression should be evaluated on
   * @tparam R
   *   class of expressions expected return type
   */
  def evaluateSeq[T <: Product: TypeTag, R: TypeTag: ClassTag](
      id: ConfigObjectId,
      configName: Option[String],
      expression: String,
      data: Seq[T]
  ...
  def syntaxCheck[T <: Product: TypeTag, R: TypeTag: ClassTag](id: ConfigObjectId, configName: Option[String], expression: String): Unit =
    try
      ...
      case e: Exception =>
        throw ConfigurationException(s"($id) spark expression syntax check for '$expression'${getConfigNameMsg(configName)} failed: ${e.getMessage}",
          configName, e)
    }
...
/**
 * DefaultExpressionData presents information from the context of the SDLB job for evaluation by
 * Spark expressions in various places of the configuration.
 */
case class DefaultExpressionData(
    feed: String,
    application: String,
    runId: Int,
    attemptId: Int,
...
object DefaultExpressionData {
  def from(context: ActionPipelineContext, partitionValues: Seq[PartitionValues]): DefaultExpressionData =
    DefaultExpressionData(
      ...
      referenceTimestamp = Timestamp.valueOf(context.referenceTimestamp),
...
private[smartdatalake] case class ExpressionEvaluationException(message: String, configurationPath: Option[String] = None, throwable: Throwable = null)
