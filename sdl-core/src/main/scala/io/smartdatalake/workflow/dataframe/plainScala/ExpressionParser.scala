...
/**
 * A simple expression parser for SQL expressions, mainly for use with Scala DataFrames.
 * Supports literals, column references, basic arithmetic and comparison operators, and function calls.
 * The parser is implemented as a recursive-descent parser with explicit precedence levels.
 *
 * Note: Code is generated by Github Copilot / GPT-5.3-Codex. Use it also to make further improvements or adjustments if needed.
 */
