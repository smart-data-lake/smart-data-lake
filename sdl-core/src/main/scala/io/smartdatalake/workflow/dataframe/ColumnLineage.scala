 ...
 * @param debugInfo         why these columns could not be traced back, collected only if there are such
 *                          columns and `Environment.columnLineageDebug` is enabled. This is diagnostic output for
