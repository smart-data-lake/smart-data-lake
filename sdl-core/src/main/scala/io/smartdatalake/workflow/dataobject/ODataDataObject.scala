...
/**
 * InversionOfControl container class This class serves as a hub or a relay for all calls to class
 * creators and to static functions outside of this module. It allows test code to override these
 * calls and to inject Mock- and Spy objects into the running code.
 */
class ODataIOC {
   ...
   * @param writer
   *   reference to an already existing Writer object
   * @return
   *   java.io.BufferedWriter object
   */
   ...
   * @param tmpDirName
   *   path to the temporary direction to write to
   * @param setup
   *   buffer configuration
   * @param context
   *   pipeline context
   * @return
   *   ODataResponseDBFSFileBuffer
   */
 ...
 *   source, it should be marked as a string in the schema to prevent casting problems.
 * @param nRetry:
 *   Optional. Number of retries after a failed attempt, default = 1
 * @param responseBufferSetup:
 *   Optional. Setup for response buffers of type [[ODataResponseBufferSetup]]
 * @param maxRecordCount:
 *   Optional. Maximum number of records to be extracted.
 */
case class ODataDataObject(
   ...
   * @param retries
   *   : Number of retries if http request fails. Default is 0 retries.
   * @return
  ...
  private def request(
    ...
    val webserviceClient = SttpWebserviceClient(
      url = url,
   ...
   *   : Received response
   * @return
   *   next link url or "" if no next link was found
   */
  ...
  /**
   * Creates the DataFrame of the received responses. This is the main method which triggers the
   * OData-API-Calls. It blocks until all data is received.
   *
    ...
    if (isRunningIncrementally(context)) {
      if (incrementalOutputExpr.isEmpty) {
        throw new ConfigurationException(
          "In execution mode DataObjectStateIncrementalMode you must configure an incrementalOutputExpr and also include it in the output schema"
        )
      }
  ...
  /**
   * * Determines if the calling action of this DataObject is running the
   * DataObjectStateIncrementalMode
   *
   * @param context
   *   the current ActionPipelineContext
   * @return
  ...
  /**
   * * This method determines the maximum value of the incrementalOutputExpr from the received data
   * and converts it to an OData representation. The resulting string can then be used for the next
   * query
   * @param df
   *   The data frame with the received data
   * @return
   *   OData representation of the new incrementalOutputExpr value
   */
