...
import io.smartdatalake.workflow.{ActionMetrics, InitSubFeed, SubFeed}
...
private[smartdatalake] trait RuntimeData {
   ...
   * If ExecutionId is empty, metrics for the current existing ExecutionId is returned.
   */
  def getRuntimeInfo(outputIds: Seq[DataObjectId], executionIdOpt: Option[ExecutionId] = None): Option[RuntimeInfo] = {
    if(executions.isEmpty) None
    else {
      if (executionIdOpt.isEmpty) currentExecution.flatMap(_.getRuntimeInfo(outputIds))
      else executions.find(_.id == executionIdOpt.get).flatMap(_.getRuntimeInfo(outputIds))
    }
  }
}
...
private[smartdatalake] case class SynchronousRuntimeData(override val numberOfExecutionToKeep: Int) extends RuntimeData {
  ...
  override def addMetric(executionId: Option[ExecutionId], dataObjectId: DataObjectId, metric: ActionMetrics): Unit = {
    ...
    assert(currentExecutionId.forall(_ == executionId.get), "New metric received for other than current ExecutionId")
...
private[smartdatalake] case class AsynchronousRuntimeData(override val numberOfExecutionToKeep: Int) extends RuntimeData {
  ...
  override def addEvent(executionId: ExecutionId, event: RuntimeEvent): Unit = {
    if (executionId.isInstanceOf[SDLExecutionId]) {
      ...
      if (execution.isEmpty && executions.isEmpty) {
        ...
        firstSDLExecutionId = Some(executionId.asInstanceOf[SDLExecutionId])
      }
  ...
  override def getRuntimeInfo(outputIds: Seq[DataObjectId], executionIdOpt: Option[ExecutionId]): Option[RuntimeInfo] = {
    if (executionIdOpt.exists(_.isInstanceOf[SDLExecutionId])) {
      ...
      if (executionIdOpt == firstSDLExecutionId) super.getRuntimeInfo(outputIds, executionIdOpt)
      else super.getRuntimeInfo(outputIds, None).map(_.copy(state = RuntimeEventState.STREAMING))
    } else {
      super.getRuntimeInfo(outputIds, executionIdOpt)
    }
  }
}
...
private[smartdatalake] case class ExecutionData[A <: ExecutionId](id: A) {
  ...
  def getRuntimeInfo(outputIds: Seq[DataObjectId]): Option[RuntimeInfo] = {
    ...
    Some(RuntimeInfo(id, lastEvent.state, startTstmp = startEvent.map(_.tstmp), duration = duration, msg = lastEvent.msg, results = results))
  }
}
...
private[smartdatalake] case class RuntimeInfo(
                                               ...
                                               msg: Option[String] = None, results:
                                               Seq[ResultRuntimeInfo] = Seq()) {
  /**
