...
abstract class SmartDataLakeBuilder extends SmartDataLakeLogger {
   ...
   * Only the init phase is executed. All data is passed directly in the form of a Spark DataFrmae from one SparkSubFeed to the next. No data is fetched from the DataObjects. Good for Unit Testing as it allows to execute your DAG without outside depedencies.
